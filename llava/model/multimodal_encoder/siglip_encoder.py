...
class SigLipVisionTower(nn.Module):
    ...
    def load_model(self, device_map=None):
        ...
        del self.vision_tower.vision_model.encoder.layers[-1:] # 27 --> 26
        self.vision_tower.vision_model.head = nn.Identity() # no attention pooling layer
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            # the last ViT layer and attention pooling layer were abandoned --> now the feature essentially comes from the second to the last layer of ViT
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
            image_features = image_forward_outs.hidden_states[-1].to(images.dtype)
            assert image_features.shape[-2] == 729

            ############ LLaVA-PruMerge ############
            # # NOTE: mm_spatial_pool_mode should be set to identity
            # prumerge_features = []
            ...
            #     #forward pass
            #     image_forward_outs = self.vision_tower(images[image_i:image_i+1].to(device=self.device, dtype=self.dtype), output_hidden_states=True)
            #     image_features = image_forward_outs.hidden_states[-1].to(images.dtype)

            #     #extract desired layer's k and q and remove hooks
            #     desired_layer_k = outputs["desired_k"] # [1, 576 + 1, 1024]
            #     desired_layer_q = outputs["desired_q"] # [1, 576 + 1, 1024]

            #     hook_handle_k.remove()
            #     hook_handle_q.remove()
                
            #     # obtain cls token by average pooling
            #     cls_features = torch.mean(desired_layer_q, dim=1).unsqueeze(1) # [1, 1, C]
            #     this_feat = self.token_prune_merge_advanced_plus(cls_features, desired_layer_k, image_features, if_adaptive=True, reduction_ratio = 1/8)
            #     prumerge_features.append(this_feat.squeeze(0))
            #     # print(this_feat.shape)
            # image_features = torch.cat(prumerge_features)
            ############ LLaVA-PruMerge ############

        return image_features

    def token_prune_merge_advanced_plus(self, cls_q, patch_k, image_features, if_adaptive=True, reduction_ratio = 1/8):
        '''
        ...
        desired_layer_k = patch_k # [1, 576 + 1, 1024]
        desired_layer_q = cls_q # [1, 576 + 1, 1024]
        ###################################################################################################################
        attn = (desired_layer_q @ desired_layer_k.transpose(-2, -1)) * C ** -0.5 # [B, N + 1, N + 1]
        attn = torch.nn.functional.softmax(attn, dim=-1) # [B, N + 1, N + 1]
        cls_attn = attn[:, 0, :] # attn[:, 0, 1:]  # [B, N], the weights of image patches with respect to cls feature
        ...
        # NOTE: this part is only difference from token_prune_merge_advanced()
        # NOTE: arithmetic_sequence introduces way many tokens and cannot achieve a good token reduction
        # if if_adaptive:
        #     step_length = int(1/reduction_ratio)
        #     arithmetic_sequence = torch.arange(0, N - 1, int(step_length/3)).to(device=self.device) # torch.arange(0, 575, int(step_length/3)).to(device=self.device)
        ...
        Key_wo_cls = desired_layer_k # desired_layer_k[:, 1:]  # [B, N-1, C]
        ...
        Key_others_norm = torch.nn.functional.normalize(Key_others, p=2, dim=-1)
        non_topk_Key_norm = torch.nn.functional.normalize(non_topk_Key, p=2, dim=-1)
