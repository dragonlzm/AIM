...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        ) # the index in flatten tensors, with each corresponding to the end index of a frame
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        boundaries: Optional[List] = [[0,1,2]],
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    hidden_states, # [1, 18864, 1536]
                    attention_mask=causal_mask, # None
                    position_ids=position_ids, # [3, 1, 18864]
            ...
            if importance_score is None: # no pruning
                ...
                if importance_score == 'decoding': # decoding phrase
                    ...
                    assert attention_mask.shape[1] > cache_att_mask.shape[1]
                    causal_mask = self._update_causal_mask(attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions)
                else: # prefill phrase
                    if importance_score == 'skip':
                        ...
                        masked_tokens = (attention_mask == False).nonzero()
                        importance_score[masked_tokens[:, 0], masked_tokens[:, 1]] = -1.0
                        importance_score[protected_inds[:, 0], protected_inds[:, 1]] = 1.0
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                # video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw).to(inputs_embeds.device) # [#frames/2 * #spatial tokens, d]
                # video_mask = input_ids == self.config.video_token_id # 151656
                # inputs_embeds[video_mask] = video_embeds

                # v_start_ind = (input_ids == self.config.video_token_id).nonzero()[:,1][0].item()
                # q_start_ind = (input_ids == self.config.video_token_id).nonzero()[:,1][-1].item() + 1
                # boundaries = [[v_start_ind, q_start_ind, input_ids.shape[1]]]
                #################### Original implementation ####################

                #################### token merging ####################
                # key implementation idea: 1. provide a shrinked video_grid_thw to create position ids of visual data, 
                #                          2. cut off redundant visual slots in input_ids, inputs_embeds, attention_mask after visual token merging,
                #                          3. remove the redundant position ids to align with inputs_embeds.
                # encode video
                video_grid_thw = torch.cat((video_grid_thw[:, 0:1], video_grid_thw[:, 1:2] * 2, video_grid_thw[:, 2:3] * 2), dim=1) # recover it
                pixel_values_videos = pixel_values_videos.type(self.visual.get_dtype())
                video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw).to(inputs_embeds.device) # [#frames/2 * #spatial tokens, d] where #spatial tokens = t * h / 4
                
                # apply token merging
                num_frames = video_grid_thw[0][0].item()
                v_feats = video_embeds.view(num_frames, -1, video_embeds.shape[-1])
                num_spatial = v_feats.shape[1]
                v_feats = bipartite_soft_matching_merge(metric=v_feats, r=num_spatial//2, x=v_feats) # 50% kept
                v_feats = bipartite_soft_matching_merge(metric=v_feats, r=num_spatial//4, x=v_feats) # 25% kept
                difference = v_feats.shape[1] - (video_grid_thw[0,1] // 2 // 2) * (video_grid_thw[0,2] // 2 // 2)
                if difference > 0: # if rope position id has fewer slots, then continue to merge visual tokens so that the number can match
                    v_feats = bipartite_soft_matching_merge(metric=v_feats, r=difference, x=v_feats) # 25% kept + removing a small number of tokens per frame
                video_embeds = v_feats.view(-1, video_embeds.shape[-1])
                
                # refresh inputs_embeds
                video_mask = input_ids == self.config.video_token_id # 151656 # (input_ids == self.config.video_token_id).nonzero().shape
                inputs_embeds[video_mask] = video_embeds
                v_inds = (input_ids == self.config.video_token_id).nonzero()[:,1]
                boundaries = [[v_inds[0].item(), v_inds[-1].item(), input_ids.shape[1]]]
                #################### token merging ####################
            else: # decoding stage
                boundaries = None
            if attention_mask is not None:
